...
parser.add_argument('--input_path', type=str,
                    default='test/hevc_sequence_cbr_frames_middle/B/BasketballDrive_1920x1080_50/',
                    help='input path to use')
parser.add_argument('--output_path', default='results/idcn/hevc_cbr_middle/BasketballDrive_1920x1080_50/',
                    type=str,
                    help='where to save the output image')
parser.add_argument('--gt_path', type=str,
                    default='test/hevc_sequence_gt_frames/B/BasketballDrive_1920x1080_50/',
                    help='input path to use')
parser.add_argument('--model', type=str, default='checkpoints/idcn_600_best.pth',
...
def load_img_helf(img_name):  # 2560×1600(A) and 1920×1080(B)
...
print(len(img_list), 'test images')  # 500
input_psnr_avg = 0
output_psnr_avg = 0
input_ssim_avg = 0
output_ssim_avg = 0
...
for i in listdir(input_path):
    if is_image_file(i):
        with torch.no_grad():
            ...
            input_img = np.zeros((h // 2 + 100, w // 2 + 100, ch))

            input_img_name_1 = '{}{}.png'.format(input_path, "%03d" % index)

            input_img[:, :, :] = load_img_helf(input_img_name_1)
            out_img = NumpyToTensor(multi_frame=False)(input_img.copy())
            ...
            out = model(input)  # have cut
            out = out.cpu()

            output_tensor = out.clone().detach()
            output_tensor = output_tensor.squeeze()  # [3, 640, 1056]
            ...
            '''拼接起来
                    if pos == 0:
                        img = img[0:height // 2 + 100, 0:weight // 2 + 100]
            '''
            output_tensor_full = torch.zeros(ch, h, w)  # h=1080, w=1920
            ...
            if opt.calc_on_y is True:
                ...
                out_img = out_img[:, :, 0]  # only test chrome channel
...
input_psnr_avg = input_psnr_avg / (index - 4)  # 头尾两张不测
