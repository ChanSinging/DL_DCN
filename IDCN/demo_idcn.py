...
parser.add_argument('--input_path', type=str,
                    default='test/hevc_sequence_cbr_frames_high/D/BasketballPass_416x240_50/',
                    help='input path to use')
parser.add_argument('--output_path', default='results/hevc_cbr_high/D/BasketballPass_416x240_50/', type=str,
                    help='where to save the output image')
parser.add_argument('--gt_path', type=str,
                    default='test/hevc_sequence_cbr_frames_high/D/BasketballPass_416x240_50/',
                    help='input path to use')
parser.add_argument('--model', type=str, default='checkpoints/best_model.pth',
                    help='rootmodel file to use')
...
if opt.submit_mode is True:
    input_root_path = 'datasets_test/hevc_cbr_low/'
    output_root_path = 'results/idcn/hevc_cbr_low/'
    gt_root_path = 'datasets/gt/'  # ground truth
                ...
                for i in listdir(input_path):
                    if is_image_file(i):
                        with torch.no_grad():
                            ...
                            input_img = np.zeros((h, w, ch))
                            input_img_name = '{}{}.png'.format(input_path, "%03d" % (index))
    ...
    for i in listdir(input_path):
        if is_image_file(i):
            with torch.no_grad():
                ...
                output_tensor = out.clone().detach()  # out是生成的RGB图
                ...
                if opt.calc_on_y is True:  # y channel
                    input_center_img = cv.cvtColor(input_center_img, cv.COLOR_BGR2YUV)
                    input_center_img = input_center_img[:, :, 0]
                    img_original = cv.cvtColor(img_original, cv.COLOR_BGR2YUV)
                    img_original = img_original[:, :, 0]
