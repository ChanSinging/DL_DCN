...
def get_training_set():
    ...
    return TrainDatasetMultiFrame(dir=root_path)


def get_validate_set():
    root_path = 'datasets/validate/'
    return ValidDataset(dir=root_path)
...
class TrainDatasetMultiFrame(data.Dataset):
    def __init__(self, dir):
        self.target_path = '{}gt/'.format(dir)
        self.input_path = '{}low_gt/'.format(dir)
...
class ValidDataset(data.Dataset):
    def __init__(self, dir):
        self.target_path = '{}sequence_gt/'.format(dir)
        self.input_path = '{}sequence_cbr_high/'.format(dir)
        self.folder_list = sorted(listdir(self.target_path))  # list 是gt_frame下的文件夹
        self.img_list = []
        for _f in self.folder_list:
            target_folder_path = '{}{}/'.format(self.target_path, _f)  # datasets/validate/gt_frame/200 || 100||
            self.img_list.append(sorted(listdir(target_folder_path)))  # datasets/validate/gt_frame/190-----200 十个路径

    def __len__(self):
        return 200

    def __getitem__(self, idx):  # idx is random
        folder_index = idx // 20
        ...
        input_img = np.zeros((5, h, w, ch))
        input_img_name_1 = '{}{}/{:0>3d}.png'.format(self.input_path, self.folder_list[folder_index], img_index - 2 + 1)
        input_img[0, :, :, :] = cv.imread(input_img_name_1)
        input_img_name_2 = '{}{}/{:0>3d}.png'.format(self.input_path, self.folder_list[folder_index], img_index - 1 + 1)
        input_img[1, :, :, :] = cv.imread(input_img_name_2)
        input_img_name_3 = '{}{}/{:0>3d}.png'.format(self.input_path, self.folder_list[folder_index], img_index + 1)
        input_img[2, :, :, :] = cv.imread(input_img_name_3)
        input_img_name_4 = '{}{}/{:0>3d}.png'.format(self.input_path, self.folder_list[folder_index], img_index + 1 + 1)
        input_img[3, :, :, :] = cv.imread(input_img_name_4)
        input_img_name_5 = '{}{}/{:0>3d}.png'.format(self.input_path, self.folder_list[folder_index], img_index + 2 + 1)
        input_img[4, :, :, :] = cv.imread(input_img_name_5)
...
# class TrainDatasetVideo(data.Dataset):
#     def __init__(self, target_path, input_path):
#         self.target_path = target_path
#         self.input_path = input_path
#         self.folder_list = sorted(listdir(self.target_path))
...
#         [target_img, input_img] = self.twin_transform([target_img, input_img])
#
#         target_tensor = transforms.ToTensor()(target_img.copy())
