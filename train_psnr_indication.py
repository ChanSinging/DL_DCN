...
# wandb setting
wandb.init(
    project="CBREN", name="original_GPU_19",
     config={
         "lr_rate": 1e-4,
        "epoch": 5000,
        "batch_size": 16
...
# Training settings
parser = argparse.ArgumentParser(description="PyTorch CBREN")
parser.add_argument("--dataset", default='datasets/', type=str, help="dataset path")
parser.add_argument("--batchSize", type=int, default=16, help="training batch size")  # default 8
...
def main():
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):
        ...
        validate(model, optimizer, epoch, validate_data_loader)
...
def train(training_data_loader, optimizer, model, criterion, epoch):
    ...
    wandb.log({'epoch': epoch, 'Loss': avr_loss})
...
def validate(present_model, optimizer, epoch_num, validate_data_loader):
    ...
    # last_psnr_10 = last_psnr_9
    # last_psnr_9 = last_psnr_8
    ...
    with torch.no_grad():
        ...
        wandb.log({'learn rate': optimizer.param_groups[0]["lr"], 'Val PSNR': psnr.avg})
        ...
        if last_psnr_1 < last_psnr_5 and last_psnr_2 < last_psnr_5 and last_psnr_3 < last_psnr_5 \
                and last_psnr_4 < last_psnr_5 and _loss < last_psnr_5:
            print("-------adjust lr-------")
            adjust_lr_flag = True
            last_psnr_1 = 0
            last_psnr_2 = 0
