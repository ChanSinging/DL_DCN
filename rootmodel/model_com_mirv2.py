...
class PyramidCell(nn.Module):
    def __init__(self, in_channels, out_channels, dilation_rates):
        ...
        self.dau_top = RCB(int(in_channels))
        self.dau_mid = RCB(int(in_channels))
        self.dau_bot = RCB(int(in_channels))
...
class DualDomainBlock(nn.Module):  # 0.5 mul sample rate
    def __init__(self, n_channels, n_pyramid_cells, n_pyramid_channels):  # 定义层
        ...
        self.channel_squeeze = Conv(in_channels=n_channels * 1, out_channels=n_channels,
                                    kernel=1, padding=0)
    ...
    def forward(self, x):  # x size = [1,64,n,n]
        ...
        _tc, _tableC, temp_tc = self.implicit_trans_2(_tc,
                                                      tc_weight)  # [4,16,16,16]--->[4,256,16,16]  [1,64,48,48]  temp_c is _tc
...
class VECNN_MF(nn.Module):  # video enhancement CNN network mul frame
    def __init__(self, n_channels=64,
        ...
        self.conv_compensate_X4 = nn.Conv2d(in_channels=n_channels, out_channels=n_channels, kernel_size=3, stride=2,
                                            padding=1)
        self.conv_compensate_X2 = nn.Conv2d(in_channels=n_channels, out_channels=n_channels, kernel_size=3, stride=2,
                                            padding=1)
    ...
    def forward(self, x):
        ...
        # print('aligned_feat :{}'.format(aligned_feat.size()))
        ...
        y = self.conv_relu_output(t_x1)  # [4,3,96,96]
