...
from cell_QG import QGConvLSTMCell
...
def net_bi_wcell(x, f, u, step, Height, Width, filter_num, kernel, relu, CNNlayer, peephole, scale):  # f foget and u update

    x1 = CNN(x, step, filter_num, filter_num, kernel, relu, CNNlayer, scale=scale, name="1")  # 空间网络 Sm(Yn)

    # print("CNN")

    inputs = tf.concat([x1, f, u], axis=-1)  # f,u are G_n
