...
class DualDomainBlock(nn.Module):  # 0.5 mul sample rate
    ...
    def forward(self, x):  # x size = [1,64,n,n]
        ...
        ty_weight = self.get_weight_y(_t)  # ty_weight from DAU
...
class DualDomainBlockCompensate(nn.Module):
    ...
    def forward(self, x, tabley, tablec, temp_ty, temp_tc):  # 利用x估计出delta_x和delta_y [16, 64, 24, 24]
        # x 是上一层IDCT的输出＋步长=2的卷积层， temp_ty是上一层的输入
        _t = x
        ...
        # tc_weight = self.get_weight_c(_t)
        _tc = self.implicit_trans_2(_tc, tablec)  # [16,64,48,48]

        _tp = self.pixel_restoration(_t)  # [16, 64, 24, 24]
        _tp = self.conv_3(_tp)  # [16, 64, 24, 24]
        _tp = self.upscale(_tp)
...
class VECNN_MF(nn.Module):  # video enhancement CNN network mul frame
    ...
    def forward(self, x):
        ...
        t_x2_temp = torch.cat((t_x2, t_x4), 1)  # [1, 128, 48, 48]
        ...
        t_x4_comp = self.compensate(t_x1_temp, t_x2_table_y, t_x2_table_c, temp_t2y, temp_t2c)  # [1,64,96,96]
