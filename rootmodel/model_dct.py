...
class GetWeight(nn.Module):
    ...
    def forward(self, x):
        b, c, size, _ = x.size()  # torch.Size([1, 64, 24, 24])
        c = c
        x = self.downsample(x)
...
class ImplicitTrans(nn.Module):  # 只是DCT变换，隐式变换
    ...
    def forward(self, x, weight):  # forward是卷积层执行层，x来自上一层的输出张量（）
        ...
        _table = new_table * self.factor + self.bias  # table is quanity coffiecent
        _kernel = self.kernel * _table
        x = x * weight  # tensor x is [4,64,16,16]

        y = F.conv2d(input=x, weight=_kernel, stride=1)  # y size:torch.Size([1, 256, 24, 24])  idct
...
class DualDomainBlock(nn.Module):  # 双域块
    def __init__(self, n_channels, n_pyramid_cells, n_pyramid_channels):  # 定义层
        ...
        self.conv_up = nn.Conv2d(in_channels=n_channels, out_channels=n_channels, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, x):  # x size = [1,64,n,n]
        ...
        ty_weight = self.get_weight_y(_t)  # 图片的张量
        ...
        _td = torch.cat([_ty, _tc], dim=1)  # [8,128,48,48]
        _td = self.conv_4(_td)  # _td:torch.Size([8, 64, 48, 48])

        y = torch.add(_td, _tp)  # value相加，size不变
        y = y.mul(0.1)

        x = self.upscale(x)  # upscale input: x
        y = torch.add(x, y)
        return y
...
class VECNN_MF(nn.Module):  # video enhancement CNN network mul frame
    ...
    def forward(self, x):
        ...
        t_x2 = torch.cat((t_x2, t_x4), 1)  # [1, 128, 48, 48]
        ...
        t_x1 = self.conv_relu_X1_2(t_x1)  # [1,64,192,192]

        y = self.conv_relu_output(t_x1)  # [4,3,64,64]  [4,3,192,192]
