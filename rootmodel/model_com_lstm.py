...
class ImplicitTransCompensate(nn.Module):
    ...
    def forward(self, delta_x, delta_y, _table):  # strict_x是量化表步长, _table是上一个尺度的量化表. delta_x and delta_y
...
class ResidualBlockNoBN(nn.Module):
    def __init__(self, num_feat=32, res_scale=1, pytorch_init=False):
...
class TSAFusion(nn.Module):  # 帧间融合
    ...
    def forward(self, aligned_feat):
        """
        Args:
            aligned_feat (Tensor): Aligned features with shape (b, t, c, h, w). t= testbach
        ...
        # lstm
        lstm_feat = self.lstm(aligned_feat)  # input size [8,64,96,96]
        embedding_ref = embedding_ref + lstm_feat  # 只对center_frame_idx帧进行lstm
...
class DualDomainBlock(nn.Module):  # 0.5 mul sample rate
    ...
    def forward(self, x):  # x size = [1,64,n,n]
        ...
        _tc, _tableC, temp_tc = self.implicit_trans_2(_tc, tc_weight)  # [4,16,16,16]--->[4,256,16,16]  [1,64,48,48]
...
class VECNN_MF(nn.Module):  # video enhancement CNN network mul frame
    ...
    def forward(self, x):
        ...
        aligned_feat = torch.stack(aligned_feat, dim=1)  # (b, t, c, h, w)==(5,8,64,96,96)

        if not self.with_tsa:  # with_tsa=true
            aligned_feat = aligned_feat.view(b, -1, h, w)  # (5,8,64,96,96)
        ...
        y = self.conv_relu_output(t_x1)  # [4,3,192,192]
