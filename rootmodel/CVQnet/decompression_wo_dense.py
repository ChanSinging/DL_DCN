...
class DPM(nn.Module):
    def __init__(self, n_channel=64):
        ...
        self.conv1 = Conv(num_feat=128)
        self.conv2 = Conv(num_feat=64)

        self.DPM_11 = DPModule(num_feat=64)
        self.DPM_21 = DPModule(num_feat=128)
        self.DPM_31 = DPModule(num_feat=256)
        self.DPM_22 = DPModule(num_feat=192)
        self.DPM_13 = DPModule(num_feat=112)
        ...
        self.up21_12 = Up()  # 64
        self.up3 = Up()  # 128
        self.out = nn.Conv2d(112, 64, kernel_size=1)
    ...
    def forward(self, x):  # x = [8,64,96,96]
        ...
        # cross M12
        # cross M22
        _M31_out = self.up1(x3)  # [40, 64, 48, 48]
        _x2 = self.conv1(x2)  # [40, 128, 48, 48]
        _x3 = torch.cat((_M31_out, _x2), dim=1)  # [40, 192, 48, 48]
        print(_x3.size())
        _x3 = self.DPM_22(_x3)  # [40, 192, 48, 48]

        # cross M13
        _M11_out_dense = self.conv2(x1)  # [40, 64, 96, 96]
        _M22_out_up = self.up3(_x3)  # # [40, 48, 48, 48]
        _M13_in = torch.cat((_M11_out_dense, _M22_out_up), dim=1)
        _M13_out = self.DPM_13(_M13_in)  # [40, 112, 96, 96]
