...
class DualDomainBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        _ty = self.conv_1(_t)  # y channel
        _tc = self.conv_2(_t)  # C Channel
...
class VECNN_MF(nn.Module):
    def __init__(self, n_channels, n_pyramids, n_pyramid_cells, n_pyramid_channels,
        ...
        # self.upscale_1 =  F.interpolate(x_center, scale_factor=4, mode='bilinear', align_corners=False)
        # self.upscale_2 = F.interpolate(x_center, scale_factor=4, mode='bilinear', align_corners=False)
    ...
    def forward(self, x):
        ...
        t_x4 = F.interpolate(t_x4, scale_factor=2, mode='bilinear', align_corners=False)
        ...
        t_x2 = F.interpolate(t_x2, scale_factor=2, mode='bilinear', align_corners=False)
        # t_x2 = self.upscale_1(t_x2, output_size=(8, 64, 96, 96))
