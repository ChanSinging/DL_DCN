...
class ImplicitTransww(nn.Module):  # 只是DCT变换，隐式变换
    ...
    def forward(self, x, weight):  # forward是卷积层执行层，x来自上一层的输出张量（）
        ...
        y = F.conv2d(input=x, weight=_kernel, stride=1)  # 卷积函数操作，conv2d首字母是小写的, 隐式i-dct变换，本质是固定权值的运算
...
class DualDomainBlock(nn.Module):   # 双域块
    def __init__(self, n_channels, n_pyramid_cells, n_pyramid_channels): # 定义层
        ...
        self.pixel_restoration = make_layer_conv(
            Conv(in_channels=n_channels, out_channels=n_channels, kernel=3, padding=1), 16)  # 此处的resblo去掉
        self.conv_3 = Conv(in_channels=n_channels, out_channels=n_channels, kernel=3, padding=1)
        self.conv_4 = Conv(in_channels=n_channels*2, out_channels=n_channels, kernel=3, padding=1)  # dct改进后in_channel 2--》8

    def forward(self, x):  # 调用层
         # x size = [4,64,16,16]
        _t = self.pyramid(x)
        _t = self.channel_squeeze(_t)
        _ty = self.conv_1(_t)
        _tc = self.conv_2(_t)
        _ty = torch.clamp(_ty, -0.5, 0.5) # [4,64,16,16]
        
    
        ty_weight = self.get_weight_y(_t)  # y通道 [4,64,1,1]
        _ty = self.implicit_trans_1(_ty, ty_weight) # forward传入的weight
        tc_weight = self.get_weight_c(_t)  # c通道
        _tc = self.implicit_trans_2(_tc, tc_weight)  # [4,16,16,16]--->[4,256,16,16]

        _tp = self.pixel_restoration(_t)  # 
        _tp = self.conv_3(_tp)
        _td = torch.cat([_ty, _tc], dim=1)  # [4,32,16,16]
        _td = self.conv_4(_td)
...
class VECNN_MF(nn.Module):  # video enhancement CNN network mul frame
    ...
    def forward(self, x):
        ...
        t_x1 = self.fusion(aligned_feat)  # [4,64,64,64]
        t_x2 = self.downscale_1(t_x1)  # [4,64,32,32]
        t_x4 = self.downscale_2(t_x2)  # [4,64,16,16]
        
        t_x4 = self.conv_relu_X4_1(t_x4) # [4, 64,16,16]
        t_x4 = self.dual_domain_blocks_x4(t_x4)  # [4,64,16,16]
        t_x4 = self.conv_relu_X4_2(t_x4)  # [4,64,16,16]
        

        t_x4 = self.upscale_2(t_x4) # [4,64,32,32]       
        t_x2 = torch.cat((t_x2, t_x4), 1)  # [4,128,32,32]       
        t_x2 = self.conv_relu_channel_merge_1(t_x2)  # [4,64,32,32]
     

        t_x2 = self.conv_relu_X2_1(t_x2) # [4,64,32,32]       
        t_x2 = self.dual_domain_blocks_x2(t_x2) # [4,64,32,32]      
        t_x2 = self.conv_relu_X2_2(t_x2)  # [4,64,32,32]
        

        t_x2 = self.upscale_1(t_x2)  # [4,64,64,64]     
        t_x1 = torch.cat((t_x1, t_x2), 1)  # [4,128,64,64]    
        t_x1 = self.conv_relu_channel_merge_2(t_x1)  # [4,64,64,64]
        

        t_x1 = self.conv_relu_X1_1(t_x1)  # [4,64,64,64]       
        t_x1 = self.dual_domain_blocks_x1(t_x1)  # [4,64,64,64]      
        t_x1 = self.conv_relu_X1_2(t_x1)  # [4,64,64,64]
        
        y = self.conv_relu_output(t_x1)  # [4,3,64,64]
